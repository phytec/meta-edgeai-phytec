...
v4l2-ctl -d /dev/cam-csi${CSI}-port${PORT} -c autogain_analogue=0,auto_exposure=1

gst-launch-1.0 v4l2src device=/dev/video-csi${CSI}-port${PORT} io-mode=dmabuf-import ! \
video/x-bayer,format=${FMT},width=${WIDTH},height=${HEIGHT},framerate=607/10 ! \
tiovxisp sink_0::device=/dev/cam-csi${CSI}-port${PORT} sensor-name=${SENSOR_NAME} \
...
! video/x-raw,format=NV12, width=${WIDTH}, height=${HEIGHT} ! queue ! tiperfoverlay title="Phycam VM-016 Object Detection" ! \
${REMOTE_SINK}
